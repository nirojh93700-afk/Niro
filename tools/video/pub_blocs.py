...
P=[
 ...
 ("cristal-v-enfant-chien.jpg","Tendres Souvenirs","Même vos animaux, gravés.","Vos plus tendres souvenirs, gravés pour toujours."),
]
...
di=tts(INTRO_VO,f"{OUT}/blocs_i.wav");segs.append(("card",card("NiV CRÉATION","Cristal Photo 3D · gravé en France").convert("RGB"),None,max(MIN,PRE+di+0.3),f"{OUT}/blocs_i.wav"))
ZW,ZH=int(W*1.18),int(H*1.18)
for i,(f,name,sub,vo) in enumerate(P):
    src=dl(f);im=Image.open(src).convert("RGB");zi=cover(im,ZW,ZH)
    ov=overlay(name,sub);dv=tts(vo,f"{OUT}/blocs{i}.wav")
    segs.append(("prod",zi,ov,max(MIN,PRE+dv+0.35),f"{OUT}/blocs{i}.wav"))
dc=tts(CTA_VO,f"{OUT}/blocs_c.wav");segs.append(("card",card("nivcreation.fr","Votre photo dans le cristal","Fait main · en France").convert("RGB"),None,max(MIN,PRE+dc+0.5),f"{OUT}/blocs_c.wav"))

# rendu
wri=imageio.get_writer(f"{OUT}/blocs_silent.mp4",fps=FPS,codec="libx264",quality=8,
...
with wave.open(f"{OUT}/blocs_audio.wav","wb") as w:
    w.setnchannels(2);w.setsampwidth(2);w.setframerate(SR);w.writeframes((sst*32767).astype(np.int16).tobytes())

subprocess.run([FF,"-y","-i",f"{OUT}/blocs_silent.mp4","-i",f"{OUT}/blocs_audio.wav",
    "-vf","scale=720:1280","-c:v","libx264","-profile:v","main","-pix_fmt","yuv420p","-crf","26","-preset","medium",
    "-c:a","aac","-b:a","150k","-movflags","+faststart","-shortest",f"{OUT}/niv-cristaux-blocs.mp4"],
    stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=True)
print("DUREE %.1fs"%total,"| taille",os.path.getsize(f"{OUT}/niv-cristaux-blocs.mp4"))
