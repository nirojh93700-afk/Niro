...
P=[
 ("bracelet-coeur-ot-dore.jpg","À Graver — Doré","Votre prénom, gravé pour vous.","Le bracelet à graver, en version dorée."),
 ("bracelet-coeur-ot-rose.jpg","À Graver — Or rose","Trois couleurs, un seul cœur.","Le bracelet à graver, en version or rose."),
 ("bracelet-coeur-ot-trio.jpg","À Graver — Trio","Doré, argenté ou or rose, à vous de choisir.","Trois couleurs disponibles, doré, argenté ou or rose."),
]
INTRO_VO="Niv Création, le bracelet cœur à graver, en trois couleurs."
CTA_VO="Faites-le graver à votre façon, sur nivcréation point f r."
...
segs=[]
di=tts(INTRO_VO,f"{OUT}/ovo_intro.wav");segs.append(("card",card("NiV CRÉATION","Le bracelet à graver").convert("RGB"),None,max(MIN,PRE+di+0.3),f"{OUT}/ovo_intro.wav"))
...
with wave.open(f"{OUT}/ot_audio.wav","wb") as w:
    w.setnchannels(2);w.setsampwidth(2);w.setframerate(SR);w.writeframes((sst*32767).astype(np.int16).tobytes())

subprocess.run([FF,"-y","-i",f"{OUT}/ot_silent.mp4","-i",f"{OUT}/ot_audio.wav",
    "-vf","scale=720:1280","-c:v","libx264","-profile:v","main","-pix_fmt","yuv420p","-crf","26","-preset","medium",
    "-c:a","aac","-b:a","150k","-movflags","+faststart","-shortest",f"{OUT}/niv-pub-bracelet-ot.mp4"],
    stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=True)
print("DUREE %.1fs"%total,"| taille",os.path.getsize(f"{OUT}/niv-pub-bracelet-ot.mp4"))
