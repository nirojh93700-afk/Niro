# -*- coding: utf-8 -*-
# Vidéo pub — nouveaux colliers (commande fournisseur Nihao, août 2026).
...
P=[
 ("collier-coeur-zircon-1.jpg","Cœur & Zircon","Un éclat doré qui attire l'œil.","Le collier cœur et zircon, un éclat doré."),
 ("collier-perle-1.jpg","Perle Solitaire","Élégance discrète au quotidien.","Le collier perle solitaire, élégance discrète."),
 ("collier-coeur-scintillant-1.jpg","Cœur Scintillant","Pour twinkle tous les jours.","Le collier cœur scintillant, pour briller au quotidien."),
]
INTRO_VO="Niv Création, les nouveaux colliers viennent d'arriver."
...
segs=[]
di=tts(INTRO_VO,f"{OUT}/cvo_intro.wav");segs.append(("card",card("NiV CRÉATION","Nouveaux colliers").convert("RGB"),None,max(MIN,PRE+di+0.3),f"{OUT}/cvo_intro.wav"))
...
dc=tts(CTA_VO,f"{OUT}/cvo_cta.wav");segs.append(("card",card("nivcreation.fr","Commandez & personnalisez","Livraison rapide & suivie").convert("RGB"),None,max(MIN,PRE+dc+0.5),f"{OUT}/cvo_cta.wav"))

wri=imageio.get_writer(f"{OUT}/colliers_silent.mp4",fps=FPS,codec="libx264",quality=8,
...
with wave.open(f"{OUT}/colliers_audio.wav","wb") as w:
    w.setnchannels(2);w.setsampwidth(2);w.setframerate(SR);w.writeframes((sst*32767).astype(np.int16).tobytes())

subprocess.run([FF,"-y","-i",f"{OUT}/colliers_silent.mp4","-i",f"{OUT}/colliers_audio.wav",
    "-vf","scale=720:1280","-c:v","libx264","-profile:v","main","-pix_fmt","yuv420p","-crf","26","-preset","medium",
    "-c:a","aac","-b:a","150k","-movflags","+faststart","-shortest",f"{OUT}/niv-pub-colliers.mp4"],
    stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=True)
print("DUREE %.1fs"%total,"| taille",os.path.getsize(f"{OUT}/niv-pub-colliers.mp4"))
