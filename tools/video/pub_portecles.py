...
P=[
 ...
 ("porte-cles-rect-demo-chien.jpg","Porte-clés Rectangle","Même vos animaux, gravés.","Le porte-clés rectangle. Même vos animaux, gravés."),
 ("porte_cles_en_cristal_avec_lumiere_led_coeur.jpg","Cœur Lumineux","S'illumine d'une pression.","Un cœur lumineux, qui s'illumine d'une simple pression."),
 ("porte_cles_en_cristal_avec_lumiere_led_rectangle.jpg","Rectangle Lumineux","Votre souvenir dans la poche.","Le rectangle lumineux, votre souvenir toujours dans la poche."),
]
INTRO_VO="Niv Création présente les porte-clés cristal lumineux."
CTA_VO="Le vôtre vous attend sur nivcréation point f r."
...
di=tts(INTRO_VO,f"{OUT}/portecles_i.wav");segs.append(("card",card("NiV CRÉATION","Porte-clés Cristal LED · gravé en France").convert("RGB"),None,max(MIN,PRE+di+0.3),f"{OUT}/portecles_i.wav"))
ZW,ZH=int(W*1.18),int(H*1.18)
for i,(f,name,sub,vo) in enumerate(P):
    src=dl(f);im=Image.open(src).convert("RGB");zi=cover(im,ZW,ZH)
    ov=overlay(name,sub);dv=tts(vo,f"{OUT}/portecles{i}.wav")
    segs.append(("prod",zi,ov,max(MIN,PRE+dv+0.35),f"{OUT}/portecles{i}.wav"))
dc=tts(CTA_VO,f"{OUT}/portecles_c.wav");segs.append(("card",card("nivcreation.fr","Votre photo, toujours avec vous","Fait main · en France").convert("RGB"),None,max(MIN,PRE+dc+0.5),f"{OUT}/portecles_c.wav"))

# rendu
wri=imageio.get_writer(f"{OUT}/portecles_silent.mp4",fps=FPS,codec="libx264",quality=8,
...
with wave.open(f"{OUT}/portecles_audio.wav","wb") as w:
    w.setnchannels(2);w.setsampwidth(2);w.setframerate(SR);w.writeframes((sst*32767).astype(np.int16).tobytes())

subprocess.run([FF,"-y","-i",f"{OUT}/portecles_silent.mp4","-i",f"{OUT}/portecles_audio.wav",
    "-vf","scale=720:1280","-c:v","libx264","-profile:v","main","-pix_fmt","yuv420p","-crf","26","-preset","medium",
    "-c:a","aac","-b:a","150k","-movflags","+faststart","-shortest",f"{OUT}/niv-cristaux-portecles.mp4"],
    stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=True)
print("DUREE %.1fs"%total,"| taille",os.path.getsize(f"{OUT}/niv-cristaux-portecles.mp4"))
