...
# (fichier ou URL complète, nom, sous-titre)
P = [
    ("https://cdn.shopify.com/s/files/1/0675/7738/0907/files/il_fullxfull.7864685300_7l25.jpg?v=1780366782", "Collier Enveloppe", "Un message secret gravé"),
    ("collier_c_ur_couple_a_graver_lot_2_pieces_or_rose.jpg", "Collier Couple Cœur", "Lot de 2, à graver"),
    ("bracelet_femme_coeur_a_graver_dore.jpg", "Bracelet Cœur", "Gravé à votre prénom"),
    ("https://cdn.shopify.com/s/files/1/0675/7738/0907/files/il_fullxfull.8007460522_d3ji.jpg?v=1780366817", "Médaillon Cœur", "S'ouvre sur vos mots"),
    ("https://cdn.shopify.com/s/files/1/0675/7738/0907/files/il_fullxfull.8086801196_rxih.jpg?v=1780366838", "Gourmette Homme", "Plaque gravée acier"),
    ("bracelet_papillon_ajoure_a_graver_argente.jpg", "Bracelet Papillon", "Ajouré et délicat"),
    ("collier-coeur-zircon-1.jpg", "Collier Cœur Zircon", "Doré et scintillant"),
    ("bracelet_cuir_tresse_a_graver_argente.jpg", "Bracelet Cuir Tressé", "Pour lui, personnalisé"),
    ("collier_medaillon_modele_livre_a_graver_argente.jpg", "Médaillon Livre", "Vos plus belles dates"),
    ("bracelet_empreinte_de_pied_de_bebe_a_graver_argente.jpg", "Bracelet Empreinte Bébé", "Souvenir de naissance"),
]
SEG_DUR = 2.2
...
def dl(f):
    ...
    dst = f"{OUT}/src_bj_{abs(hash(url))}.img"
    subprocess.run(["curl", "-sL", "--max-time", "60", "-o", dst, url], check=True)
    return dst
...
segs = []
segs.append(("card", card("NiV CRÉATION", "Nos bijoux à graver").convert("RGB"), None, INTRO_DUR))
ZW, ZH = int(W * 1.18), int(H * 1.18)
for f, name, sub in P:
    src = dl(f); im = Image.open(src).convert("RGB"); zi = cover(im, ZW, ZH)
    ov = overlay(name, sub)
    segs.append(("prod", zi, ov, SEG_DUR))
segs.append(("card", card("nivcreation.fr", "Personnalisez le vôtre", "Gravé en France · Livraison suivie").convert("RGB"), None, OUTRO_DUR))

# rendu vidéo (silencieux : aucune piste audio)
wri = imageio.get_writer(f"{OUT}/bijoux_silencieuse.mp4", fps=FPS, codec="libx264", quality=8,
...
subprocess.run([FF, "-y", "-i", f"{OUT}/bijoux_silencieuse.mp4",
    "-vf", "scale=720:1280", "-c:v", "libx264", "-profile:v", "main", "-pix_fmt", "yuv420p",
    "-crf", "26", "-preset", "medium", "-an", "-movflags", "+faststart",
    f"{OUT}/niv-pub-bijoux-mix.mp4"],
    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
print("DUREE %.1fs" % total, "| taille", os.path.getsize(f"{OUT}/niv-pub-bijoux-mix.mp4"))
