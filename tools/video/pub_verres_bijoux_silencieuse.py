...
# Sélection du 22/09/2026 : les VERRES et la CARAFE n'avaient jamais été filmés
# (vérifié sur les 16 scripts de tools/video/), plus 4 bijoux jamais montrés.
P = [
    ("carafe_gravee.jpg", "Carafe à whisky gravée", "Le prénom, la date, le millésime"),
    ("verre_a_whisky_grave_ambiance.jpg", "Verre à whisky", "Gravé sur la face et au fond"),
    ("verre_vin_grave.jpg", "Verre à vin gravé", "À l'unité, par 2 ou par 4"),
    ("flute_ambiance.jpg", "Flûte à champagne gravée", "Pour les grandes occasions"),
    ("collier-double-coeur-1.jpg", "Collier Double Cœur", "Deux prénoms, recto et verso"),
    ("collier-coeur-grave-1.jpg", "Collier Cœur", "Gravé recto-verso"),
    ("collier-3coeurs-1.jpg", "Collier 3 Cœurs entrelacés", "Trois gravures, trois histoires"),
    ("collier-coeur-plaques-1.jpg", "Collier Cœur & 2 plaques", "Trois zones à personnaliser"),
    ("bracelet-cordon-plaque-1.jpg", "Bracelet cordon à plaque", "Plaque gravée, cordon ajustable"),
]
SEG_DUR = 2.2
...
segs = []
segs.append(("card", card("NiV CRÉATION", "Verres, carafes & bijoux gravés").convert("RGB"), None, INTRO_DUR))
...
wri = imageio.get_writer(f"{OUT}/verres_bijoux_silencieuse.mp4", fps=FPS, codec="libx264", quality=8,
...
subprocess.run([FF, "-y", "-i", f"{OUT}/verres_bijoux_silencieuse.mp4",
    "-vf", "scale=720:1280", "-c:v", "libx264", "-profile:v", "main", "-pix_fmt", "yuv420p",
    "-crf", "26", "-preset", "medium", "-an", "-movflags", "+faststart",
    f"{OUT}/niv-verres-et-bijoux.mp4"],
    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
print("DUREE %.1fs" % total, "| taille", os.path.getsize(f"{OUT}/niv-verres-et-bijoux.mp4"))
