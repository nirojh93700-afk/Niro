...
P=[
 ("collier_c_ur_couple_a_graver_lot_2_pieces_or_rose.jpg","Collier Couple Cœur","Votre amour, gravé.","Le collier couple cœur, votre amour gravé."),
 ("bracelet_femme_coeur_a_graver_dore.jpg","Bracelet Cœur","Tendre et personnalisé.","Le bracelet cœur, tendre et personnalisé."),
 ("collier_medaillon_modele_livre_a_graver_dore.jpg","Médaillon Livre","Vos dates les plus belles.","Le médaillon livre, vos plus belles dates."),
 ("bougeoir-lotus-1.jpg","Bougeoir Lotus","Une ambiance zen.","Le bougeoir fleur de lotus, une ambiance zen."),
 ("bougeoir-mandala-1.jpg","Bougeoir Flottant","Déco au motif mandala.","Le bougeoir flottant, au motif mandala."),
 ("arbre-vie-lumineux-2.jpg","Lampe Arbre de Vie","Lumière douce en bois.","La lampe Arbre de Vie, une lumière douce."),
 ("couverts_enfants_ex_flatlay.jpg","Couverts Enfants","Gravés au prénom.","Les couverts enfants, gravés au prénom."),
 ("numero_table_rectangulaire_feuillage_bois.png","Numéro de Table","Sublimez votre mariage.","Le numéro de table, pour votre mariage."),
 ("verre_a_whisky_card.jpg","Verre Gravé","Un cadeau qui marque.","Le verre gravé, un cadeau qui marque."),
]
...
# préparer segments
segs=[]  # (kind, image_zoomed_or_card, overlay_or_None, dur, vo)
di=tts(INTRO_VO,f"{OUT}/fvo_intro.wav");segs.append(("card",card("NiV CRÉATION","L'atelier français de gravure").convert("RGB"),None,max(MIN,PRE+di+0.3),f"{OUT}/fvo_intro.wav"))
ZW,ZH=int(W*1.18),int(H*1.18)
for i,(f,name,sub,vo) in enumerate(P):
    src=dl(f);im=Image.open(src).convert("RGB");zi=cover(im,ZW,ZH)
    ov=overlay(name,sub);dv=tts(vo,f"{OUT}/fvo{i}.wav")
    segs.append(("prod",zi,ov,max(MIN,PRE+dv+0.35),f"{OUT}/fvo{i}.wav"))
dc=tts(CTA_VO,f"{OUT}/fvo_cta.wav");segs.append(("card",card("nivcreation.fr","Commandez & personnalisez","Fait main · en France").convert("RGB"),None,max(MIN,PRE+dc+0.5),f"{OUT}/fvo_cta.wav"))

# rendu
wri=imageio.get_writer(f"{OUT}/free_silent.mp4",fps=FPS,codec="libx264",quality=8,
...
# audio : voix + beat + nappe
total=sum(d for *_,d,_ in [(s[0],s[3],s[4]) for s in segs]) if False else sum(s[3] for s in segs)
...
with wave.open(f"{OUT}/free_audio.wav","wb") as w:
    w.setnchannels(2);w.setsampwidth(2);w.setframerate(SR);w.writeframes((sst*32767).astype(np.int16).tobytes())

subprocess.run([FF,"-y","-i",f"{OUT}/free_silent.mp4","-i",f"{OUT}/free_audio.wav",
    "-vf","scale=720:1280","-c:v","libx264","-profile:v","main","-pix_fmt","yuv420p","-crf","26","-preset","medium",
    "-c:a","aac","-b:a","150k","-movflags","+faststart","-shortest",f"{OUT}/niv-pub-gratuite.mp4"],
    stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=True)
print("DUREE %.1fs"%total,"| taille",os.path.getsize(f"{OUT}/niv-pub-gratuite.mp4"))
