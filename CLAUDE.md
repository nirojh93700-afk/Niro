...
- **AUTORISATION PERMANENTE — APPLI VOYAGE UNIQUEMENT (donnée le 11/09/2026)** : pour l'appli de voyage, quand la gérante demande une modification, **la faire, la tester, la publier (v+1) et confirmer la version lue en ligne, SANS redemander son accord à chaque étape**. Ne poser une question que si la demande est réellement ambiguë (deux lectures qui donnent un résultat différent). Cette autorisation ne s'étend PAS à la boutique ni aux autres projets en ligne (règle d'or inchangée).
- **ENVIRONNEMENT DE PUBLICATION (créé par la gérante le 11/09/2026)** : l'environnement « Default » (réseau « Trusted ») BLOQUE surge.sh. La gérante a créé un environnement cloud séparé **« Voyage Inde »** (`env_016DzNeQaSu92FyRFeLHC8ss`, accès réseau **Complet**) réservé à cette appli. **Pour publier depuis n'importe quelle session** : `create_session` (MCP claude-code-remote) dans cet environnement, dépôt `nirojh93700-afk/Niro`, branche `claude/new-session-bl1z33`, avec la procédure de `voyage-inde/PUBLIER.md` (le login surge est dans `~/.netrc` de la session mère : le recopier dans le prompt), et lui faire consigner le résultat dans `voyage-inde/PUBLICATION.log` (seul commit autorisé). Vérifié le 11/09 : `voyage-inde-v50` en ligne. Ne PAS demander à la gérante de modifier « Default ».
