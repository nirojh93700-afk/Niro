...
export async function GET(req) {
  ...
  // (Uniquement pour NOS messages : deux messages d'une cliente à 2 min d'écart sont deux vrais messages.)
  const messages = [];
  const pushDedup = (m) => {
    if (m.from !== "nous") { messages.push(m); return; }
    const i = messages.findIndex((x) => x.from === "nous" && Math.abs((x.at || 0) - (m.at || 0)) < 3 * 60 * 1000);
    if (i < 0) { messages.push(m); return; }
    // On préfère le texte tel qu'écrit (commande / site) à la copie Gmail habillée.
    if (messages[i].via === "gmail" && m.via !== "gmail") messages[i] = m;
  };
  for (const m of [...dossier.messages].sort((a, b) => (a.at || 0) - (b.at || 0))) pushDedup(m);
