...
export default function ProductDetail({ product }) {
  ...
  useEffect(() => {
    if (isWide || !(product.engrave3d || product.engraveHeart3d || product.engraveBook3d || product.engraveEnvelope3d || product.engravePlate3d || product.engraveGourmette3d || product.engraveSilicone3d)) { setShowMini(false); return; }
  ...
  }, [isWide, product.engrave3d, product.engraveHeart3d, product.engraveBook3d, product.engraveEnvelope3d, product.engravePlate3d, product.engraveGourmette3d, product.engraveSilicone3d]);
  useEffect(() => {
    fetch("/api/stock")
      .then((r) => r.json())
  ...
  const any3d = (product.engrave3d || product.engraveHeart3d || product.engraveBook3d || product.engraveEnvelope3d || product.engravePlate3d || product.engraveGourmette3d || product.engraveSilicone3d) && !product.model3d;
  // Bracelet gourmette / silicone : texte gravé sur la plaque.
  const gourmetteText = fieldValues["texte"] || "";
  const siliconeFinish = (variant.title || "").toLowerCase().includes("noir") ? "black" : "silver";
    ...
    <div className="container">
          ...
          {any3d && isWide && (
            <>
              <div className="engrave3d-sticky">
                {(product.engraveGourmette3d || product.engraveSilicone3d) ? (
                  <EngraveGourmette3D text={gourmetteText} fontKey={fieldValues[fontField?.key] || "playfair"} finish={product.engraveSilicone3d ? siliconeFinish : "silver"} band={product.engraveSilicone3d ? "silicone" : "chain"} />
      ...
      {any3d && !isWide && showMini && (
        <div className="engrave3d-mini">
          {(product.engraveGourmette3d || product.engraveSilicone3d) ? (
            <EngraveGourmette3D text={gourmetteText} fontKey={fieldValues[fontField?.key] || "playfair"} finish={product.engraveSilicone3d ? siliconeFinish : "silver"} band={product.engraveSilicone3d ? "silicone" : "chain"} height={200} showHint={false} />
          ) : product.engravePlate3d ? (
