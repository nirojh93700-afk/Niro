...
| **Favoris enregistrés dans le compte client** (15/09/2026) | `docs/maquettes/favoris-compte.html` (générée par `npm run maquette-favoris`) | **Maquette faite, EN ATTENTE DE SA VALIDATION.** Chantier demandé : « les clients mettent en favoris et ça va directement dans leur compte ». Existe déjà : le ♡ (`WishlistButton.jsx`) + `/favoris` mais en **localStorage** (par navigateur), et l'espace client `/espace` (connexion par lien magique, cookie signé 30 j). À construire : favoris rangés côté serveur par e-mail, **fusion des favoris du navigateur à la connexion**, et écran Gestion → Favoris (top produits + clientes + export). Aucun e-mail automatique. |
