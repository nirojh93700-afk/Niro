...
| **Valérine Zaghini** (`valerinezaghini@gmail.com`) — #0URL2JQA | ✅ **ENVOYÉ le 15/09** (Messages clients). ⏰ **À SURVEILLER : date gravée 26/09**, « ce sera juste » + suivi promis → **lui écrire AVANT le 26 si intenable** | `src/lib/messagesPrets.js` (historique) |
...
| **Rose Catarino** (`rose75013@icloud.com`) — #1Z17IKQ8 | ✅ **ENVOYÉ le 15/09** — **attendre sa réponse** (français ou portugais ?) avant de graver ; **aperçu promis avant gravure** | `src/lib/messagesPrets.js` (historique) |
...
| **Bouton « Répondre » dans les e-mails clients** (15/09) | `docs/maquettes/bouton-repondre-email.html` · artifact https://claude.ai/artifact/61VgP7rAt9WtZcKfdz9B3k | **Demande du gérant du 15/09, maquette à valider.** Bouton doré dans chaque e-mail client (Messages clients + fil Communications & aperçu) → page de réponse à la marque → la réponse tombe dans le DOSSIER de la cliente + fil de commande + alerte. Réutilise jetons /repondre + logComm + batAtelierMessage. NB : les réponses e-mail classiques sont DÉJÀ rangées par la boîte surveillée — le bouton rend le chemin direct et immédiat. Quand il dit « applique » : construire (page + POST + bouton dans emailLayout des envois clients). |
