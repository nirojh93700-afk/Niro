...
| **Alicia** (`alicia.ilyess@gmail.com`) — verre whisky, gravure du FOND, anniversaire 28/09 | (fil dans son dossier comms) | ✅ Répondu le 17/09 par le site (validation `/api/reply`) sur ordre du gérant : gravure du fond POSSIBLE, texte trop long → lui demander une version 4-6 mots, et PAS pour le 26/09 → possible la semaine d'APRÈS. Elle a répondu le 17/09 (texte court « J'aime ta bite 💜 » ✓, délai accepté, demandait le prix) → ✅ 2e réponse envoyée le 17/09 au soir (« envoie ») : 19,90 € gravure comprise, fond = emplacement sans supplément, les 3 étapes pour commander sur la fiche, AUCUNE mention de délai (le site l'affiche tout seul). ✅ **Elle a COMMANDÉ le 17/09 (#1JXNFE0L, 29,81 €, BIENVENUE10, cadeau « plutôt femme »)** — champs vides, précision par e-mail : NE PAS graver le modèle de base, UNIQUEMENT « J'aime ta bite » + petit cœur au FOND (note posée sur la commande le 18/09). Confirmation programmée 18/09 10 h (`sch_bma7rh22mu6j9dt9`, validée par le gérant). Plus rien en attente côté cliente. |
...
| **Ophélie Terraz** (#0KVSBXKR, `ophelie.terraz@gmail.com`) — 4 verres modèle 28, colis direct chez sa maman (Montbéliard) | — | ✅ Répondu le 17/09 par le site (« envoie » du gérant) : cadeau surprise → **collier cœur proposé** (sa question du 12/09 restée 5 jours sans réponse). Attendre son choix (« d'accord pour le collier cœur » / surprise). ⚠️ Si elle prend le collier cœur : le glisser dans le colis à l'expédition. |
